...
'''Arguments: Roach, ADC board, Channel, RecordData
    Uses different registers than Version 2
'''
...
def trigger():
    ...
    '''#time.sleep(0.001)
    roach.write_int('adc_data_adc_ctrl',0)
    roach.write_int('adc_data_adc_ctrl',7)
...
def grab_spec(roach,n=100,adc=0,chan=0,):
    ...
    bram = 'adc_data_adc_bram'
    
    #Set register value (sel) for correct ADC board and Channel
    #adc0_ch0 = 0, adc0_ch1 = 1, adc1_ch0 = 2, adc1_ch1 = 3,
    if int(adc) == 0:
        sel = int(chan)
    elif int(adc) == 1:
        sel = int(chan) + 2
    roach.write_int('swreg_snap_select',sel)
...
def plot_callback(channel, board, roach):
    ...
    try:
        if int(board):
            attnQ = ((roach.read_int('swreg_adc_atten',0) & 0xff)) / 2.0
            attnI = ((roach.read_int('swreg_adc_atten',0) & 0xff00) >> 8) / 2.0
           #print attnI, attnQ
        else:
            attnQ = ((roach.read_int('swreg_adc_atten',0) &  0xff0000) >> 16) / 2.0
            attnI = ((roach.read_int('swreg_adc_atten',0) & 0xff000000) >> 24) / 2.0
           #print attnI, attnQ
            
            
        if channel == 'I' or channel == 'both':
            if rec:
                df = open('/archive/eovsa/Katadc_snapshot_data/'+roach_ip[:6]+'_adc'+board+'_'+datetime.datetime.now().strftime('%Y_%m_%d_%H:%M:%S')+'_I'+str(attnI)+'db.dat', 'w')
        ...
        if channel == 'Q' or channel == 'both':
            if rec:
                df = open('/archive/eovsa/Katadc_snapshot_data/'+roach_ip[:6]+'_adc'+board+'_'+datetime.datetime.now().strftime('%Y_%m_%d_%H:%M:%S')+'_Q'+str(attnQ)+'db.dat', 'w')
        ...
        subplots[1].set_title('Q Channel Attenuation = '+str(attnQ)+', I Channel Attenuation = '+str(attnI))
