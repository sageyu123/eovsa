...
from eovsapy.util import *
from collections import deque
from math import fmod, pi
from . import antenna_control as ant_ctrl
from eovsapy import eovsa_lst as el
...
#                    print 'Update took',time.time()-t,'seconds.'
    ...
    def update_ant(self,data,iant):
        ...
        for text, color in az_stat.get(list(range(24))):
            if text:
                ...
                j += 1
        el = extract(data,ant['ElevationStatus'])
        el_stat = ElStatus(el)

        # Alert if drive is currently in a Trip state
        if el_stat.bits[0] == 1:
            # Azimuth drive is tripped, so display Trip info
            tripcode = extract(data,ant['ElevationTrip0'])
            tripinfo = ant_ctrl.get_trip(tripcode)
        ...
        for text, color in el_stat.get(list(range(24))):
            if text:
                ...
                j += 1
        cs = extract(data,ant['CentralStatus'])
        ...
        for j,key in enumerate(keys):
            value = str(extract(data,ant[key]))
    ...
    def update_display(self,data):
        ...
        t = Time(extract(data,sf['Timestamp'])+0.000001,format='lv')
        mjd = t.mjd
        version = extract(data,sf['Version'])
        line = 'SF v'+str(version)+' Time: '+t.iso
        task = extract(data,sf['Schedule']['Task']).strip('\x00').replace('\t',' ').replace('\r\n','|')
        if task == '':
            task = self.last_task
        else:
        ...
        new_line = 'Weather>>'
        new_line += ' Wind: ' + str(int(extract(data,weather['Wind']))) + ' mph ' + '<'+str(int(extract(data,weather['AvgWind'])))+'>'
        dirs = ['N ','NE','E ','SE','S ','SW','W ','NW']
        direction = extract(data,weather['WindDirection'])/dtor
        # Convert compass reading to cardinal direction
        idir = int(fmod(direction+22.5,360.)/45.)
        new_line += ' from ' + dirs[idir]
        new_line += '  Temp: ' + str(extract(data,weather['Temperature']))[:4] + ' F'
        pressure = extract(data,weather['Pressure'])
        new_line += '  Pressure: ' + str(pressure)[:6] + ' mbar'
        cr_temp = int(extract(data,sf['Schedule']['Data']['Roach'][0]['Temp.ambient'])*90./5)/10. + 32
        ...
        new_line = 'SolPwr12>>'
        sptime = Time(extract(data, solpwr12['Timestamp']),format='lv')
        nsec = (t - sptime).value*86400
        if nsec > 120.0:
            new_line += 'Last read '+sptime.iso[:19]+', '+str(int(nsec))+'s'
            charge12 = 0.0
        else:
            charge12 = extract(data,solpwr12['Charge'])
            new_line += ' Charge: ' + str(charge12)+'%  Volts:'+str(extract(data,solpwr12['Volts']))[:4]+'VDC  Amps:'+str(extract(data,solpwr12['Amps']))[:4]+'A'
        new_line += '  SolPwr13>>'
        sptime = Time(extract(data, solpwr13['Timestamp']),format='lv')
        nsec = (t - sptime).value*86400
        if nsec > 120.0:
            new_line += 'Last read '+sptime.iso[:19]+', '+str(int(nsec))+'s'
            charge13 = 0.0
        else:
            charge13 = extract(data,solpwr13['Charge'])
            new_line += ' Charge: ' + str(charge13)+'%  Volts:'+str(extract(data,solpwr13['Volts']))[:4]+'VDC  Amps:'+str(extract(data,solpwr13['Amps']))[:4]+'A'
        self.L3.insert(END,new_line)
        ...
        sub1 = extract(data,sf['LODM']['Subarray1'])
        sub2 = extract(data,sf['LODM']['Subarray2'])
        subarray1 = []
        subarray2 = []
        for i in range(16):
        ...
        for i in altantindex:
            if subarray1[i]:
                ...
                nsubarray += 1
                ra = RA_Angle(extract(data,c['RAVirtualAxis'])/10000.,'degrees')
                dec = Dec_Angle(extract(data,c['DecVirtualAxis'])/10000.,'degrees')
        ...
        if not self.sectionDisplayState[0]:
            ...
            for i in altantindex:
                ...
                tostow = extract(data,c['CentralStatus']) & 256
                ...
                toservice = (int(azel['RequestedAzimuth'][i] + 0.5) == 225 and 
                             ...
                             rmode[extract(data,c['RunMode'])] == 'POSITION')
                ...
                bs = extract(data,fe['BrightScram']['State'])
                ws = extract(data,fe['WindScram']['State'])
                if bs:
                    line += "    BRIGHTSCRAM    "
                elif ws:
                    line += "     WINDSCRAM     "
                ...
                azoff, eloff = extract(data,c['AzOffset'])/10000., extract(data,c['ElOffset'])/10000.
                raoff, decoff = extract(data,c['RAOffset'])/10000.,extract(data,c['DecOffset'])/10000.
                if i > 12:
        ...
        for i in antindex:
            nsubarray += 1
            c = sf['Antenna'][i]['Controller']
            criomjd = int(mjd) + extract(data,c['cRIOClockms'])/86400000.
            ...
            antmjd = min(extract(data,c['SystemClockMJDay']),int(mjd)) + extract(data,c['SystemClockms'])/86400000.
        ...
        if not self.sectionDisplayState[1]:
            ...
            for i in antindex:
                ...
                criotdif = (criomjd - mjd)*86400.
                anttdif = (antmjd - mjd)*86400.
                line = antn[i]+pswitch[np.clip(extract(data,c['PowerSwitch']),0,2)]
                line += "  "+rctrl[np.clip(extract(data,c['RunControl']),0,2)]
                line += "  "+rmode[np.clip(extract(data,c['RunMode']),0,5)]
                line += "  "+dmode[np.clip(extract(data,c['DataMode']),0,2)]
        ...
        errstr = extract(data,sf['LODM']['LO1A']['ERR']).split('\n')[0]
        ...
        if len(errvals) == 2:
            ...
            errno = 0
            errmsg = errvals[0]
        stat = extract(data,sf['LODM']['LO1A']['SweepStatus'])
        statdict = {0:'Stopped',8:'Sweeping',32:'Wait4Trg'}
        fseqfile = extract(data,sf['LODM']['LO1A']['FSeqFile']).strip('\x00')
        try:
        ...
        if not self.sectionDisplayState[2]:
            ...
            esr = str(extract(data,sf['LODM']['LO1A']['ESR']))
            stb = str(extract(data,sf['LODM']['LO1A']['STB']))
        ...
        if scan_state == -1:
            ...
            stateline = 'ON '
        if extract(data,c['PhaseTracking']) == 0:
            trackline = 'False'
        else:
            trackline = 'True '
        ...
        if not self.sectionDisplayState[3]:
            ...
            self.L3.insert(END,head)
            self.L3.itemconfig(END,bg=self.colors['colhead'])
            uvw = extract(data,c['UVW'])
            delay = extract(data,c['Delay'])
            for i in antindex:
        ...
        for i in antindex:
            if subarray1[i]:
                ...
                hpv = extract(data,fe['HPol']['Power'])
                vpv = extract(data,fe['VPol']['Power'])
                if hpv == 0.0 or vpv == 0.0: nfbad += 1
                be = sf['DCM'][i]
                hpv = extract(data,be['HPol']['Voltage'])
                vpv = extract(data,be['VPol']['Voltage'])
        ...
        if not self.sectionDisplayState[4]:
            ...
            for i in antindex:
                ...
                vpv = extract(data,fe['VPol']['Power'])
                ndon = ' '
                if extract(data,fe['ND']) == 1:
                    ndon = '**ND-ON**'
                be = sf['DCM'][i]
                line = '{:2d}  {:>9}  {:7.4f} {:3d} {:3d} {:7.4f} {:3d} {:3d}            {:7.4f} {:5d}   {:7.4f} {:5d}'.format(i+1,ndon,
                    hpv,extract(data,fe['HPol']['Attenuation']['First']),
                    extract(data,fe['HPol']['Attenuation']['Second']),
                    vpv,extract(data,fe['VPol']['Attenuation']['First']),
                    extract(data,fe['VPol']['Attenuation']['Second']),
                    extract(data,be['HPol']['Voltage']),extract(data,be['HPol']['Attenuation']),
                    extract(data,be['VPol']['Voltage']),extract(data,be['VPol']['Attenuation']))
                self.L3.insert(END,line)
                ...
                if not subarray1[i]:
                    # Antennas out of service (color gray)
                    self.L3.itemconfig(END,bg=self.colors['na'])
                if extract(data,fe['HPol']['Attenuation']['First']) !=0 or extract(data,fe['VPol']['Attenuation']['First']) !=0:
                    # First attenuator is non zero (possible problem or burst). 
                    self.L3.itemconfig(END,bg=self.colors['warn'])
                if np.isnan(extract(data,fe['HPol']['Power'])) or np.isnan(extract(data,fe['VPol']['Power'])):
        ...
        for i in antindex:
            if subarray1[i]:
                ...
                lairdtemp = extract(data,laird['Temperature'])
                if  lairdtemp < 24 or lairdtemp > 26: nlbad += 1
                fe = sf['Antenna'][i]['Frontend']['FEM']
                fetemp = extract(data,fe['Temperature'])
        ...
        if not self.sectionDisplayState[5]:
            ...
            for i in antindex:
                ...
                line = '{:2d}     {:6.3f}       {:6.3f}     {:6.3f}   {:8.3f}  {:7.3f}  {:6.3f}   {:3d}     {:3d}     {:3d}'.format(i+1,
                    fetemp,lairdtemp,extract(data,laird['TReference']),
                    extract(data,laird['DutyFactor']),extract(data,laird['InputVoltage']),
                    extract(data,laird['MainCurrent']),extract(data,laird['Alarm']),
                    extract(data,laird['Error']),extract(data,laird['ErrorHistory']))
                if i == 13:
                    lairdtemp = extract(data,sf['FEMA']['Thermal']['SecondStageTemp'])
        ...
        for i in range(nroaches):
            fpgafan = extract(data,r[i]['Fan.fpga'])
        ...
        if not self.sectionDisplayState[6]:
            ...
            for i in range(nroaches):
                v1 = str(extract(data,r[i]['Voltage.1v'])+0.05)[:3]
                v15 = str(extract(data,r[i]['Voltage.1v5'])+0.05)[:3]
                v18 = str(extract(data,r[i]['Voltage.1v8'])+0.05)[:3]
                v25 = str(extract(data,r[i]['Voltage.2v5'])+0.05)[:3]
                v33 = str(extract(data,r[i]['Voltage.3v3'])+0.05)[:3]
                v5 = str(extract(data,r[i]['Voltage.5v'])+0.05)[:3]
                v12 = str(extract(data,r[i]['Voltage.12v'])*3+0.05)[:4]
                c1 = str(extract(data,r[i]['Current.1v'])+0.05)[:3]
                c15 = str(extract(data,r[i]['Current.1v5'])+0.05)[:3]
                c18 = str(extract(data,r[i]['Current.1v8'])+0.05)[:3]
                c25 = str(extract(data,r[i]['Current.2v5'])+0.05)[:3]
                c33 = str(extract(data,r[i]['Current.3v3'])+0.05)[:3]
                c5 = str(extract(data,r[i]['Current.5v'])+0.05)[:3]
                c12 = str(extract(data,r[i]['Current.12v'])+0.05)[:3]
                tamb = str(extract(data,r[i]['Temp.ambient'])+0.5).split('.')[0]
                tfpga = str(extract(data,r[i]['Temp.fpga'])+0.5).split('.')[0]
                tin = str(extract(data,r[i]['Temp.inlet'])+0.5).split('.')[0]
                tout = str(extract(data,r[i]['Temp.outlet'])+0.5).split('.')[0]
                tppc = str(extract(data,r[i]['Temp.ppc'])+0.5).split('.')[0]
                dla0x = extract(data,r[i]['Delay0x'])
                dla0y = extract(data,r[i]['Delay0y'])
                dla1x = extract(data,r[i]['Delay1x'])
                dla1y = extract(data,r[i]['Delay1y'])
                ...
                if fpgafan == 0:
                    self.L3.itemconfig(END,bg=self.colors['na'])
                status = hex(extract(data,r[i]['Status']))
        ...
        if not self.sectionDisplayState[7]:
            ...
            for i in antindex:
                ...
                an='  '+'{:2d}'.format(i+1)+":   "
                self.L3.insert(END,an+extract(data,parser['Command']).strip('\x00')+' '+str(extract(data,parser['CommErr'])))
    ...
    def cryo_display(self,data):
        ...
        for k,cryo in enumerate(cryos):
            ps = cryo['PowerStrip']
            lvtstamp = extract(data,cryo['Timestamp'])
            if int(lvtstamp) == 0:
                line = 'Time: 1904-01-01 00:00:00.000   Version: '+'{:4.1f}'.format(extract(data,cryo['Version']))
            else:
                line = 'Time: '+Time(lvtstamp,format='lv').iso+'   Version: '+'{:4.1f}'.format(extract(data,cryo['Version']))
            ...
            self.cryoLB.insert(END,line)
            self.cryoLB.itemconfig(END,bg=self.colors['colhead'])
            cur = extract(data,ps['Current'])
            vol = extract(data,ps['Volts'])
            line = '{:>5.0f} {:>5.0f}  {:>5.1f} {:>5.1f}  '.format(vol[0],vol[1],cur[0],cur[1])
            line += '  '+onoff[extract(data,ps['ComputerStatus'])]+'  '+onoff[extract(data,ps['DeltaTauBrickStatus'])]
            line += '  '+onoff[extract(data,ps['LNA5VBiasBBStatus'])]+'  '+onoff[extract(data,ps['LNA12VBiasStatus'])]
            line += '  '+onoff[extract(data,ps['SecondAmpsStatus'])]+'  '+onoff[extract(data,ps['OpticalTxRabbitStatus'])]
            line += '  '+onoff[extract(data,ps['RFSwitchStatus'])]+'  '+onoff[extract(data,ps['NoiseDiodeStatus'])]
            self.cryoLB.insert(END,line)
            # Section on Receiver
            rx = cryo['Receiver']
            hifreq = 'Disabled'
            lofreq = 'Disabled'
            nd = 'OFF'
            if extract(data,rx['HiFreqEnabled']):
                hifreq = 'Enabled '
            if extract(data,rx['LoFreqEnabled']):
                lofreq = 'Enabled '
            if extract(data,rx['NoiseDiodeEnabled']):
                nd = 'ON '
            ...
            for x in lna_order:
                ...
                line = '{:4s}   {:8.2f}      {:8.2f}      {:8.2f}      {:8.2f}      {:8.2f}      {:8.2f}'.format(x,
                    extract(data,lna['DrainVoltage']),extract(data,lna['DrainCurrent']),
                    extract(data,lna['GateAVoltage']),extract(data,lna['GateACurrent']),
                    extract(data,lna['GateBVoltage']),extract(data,lna['GateBCurrent']))
                self.cryoLB.insert(END,line)
            # Section on Temperatures
            rt = cryo['Thermal']
            fe14temp = extract(data,sf['Antenna'][13]['Frontend']['FEM']['Temperature'])
            ...
            line += '   {:5.1f}  '.format(extract(data,rt['RadiationShieldTemp']))
            line += '   {:5.1f}  '.format(extract(data,rt['SecondStageTemp']))
            line += '   {:5.1f}   '.format(extract(data,rt['FirstStageTemp']))
            line += '   {:5.1f}    '.format(extract(data,rt['HiFreq15KPlateTemp']))
            line += '   {:5.1f}   '.format(extract(data,rt['LowFreqFeedhornTemp']))
            line += '   {:5.1f}   '.format(extract(data,rt['HiFreqFeedhornTemp']))
            line += '   {:5.1f}  '.format(extract(data,rt['LowFreqLNATemp']))
            line += '   {:5.1f}  '.format(extract(data,rt['HiFreqLNATemp']))
            self.cryoLB.insert(END,line)
            # Section on Motors
            serv = cryo['FRMServo']
            selrx = '***Unset***'
            if extract(data,serv['SelectedRx']) == 1:
                selrx = 'Low Freq RX '
            elif extract(data,serv['SelectedRx']) == 2:
                selrx = 'High Freq RX'
            homed = '       '
            if extract(data,serv['Homed']) == 1: homed = '*HOMED*'
            ...
            for j,motor in enumerate(motors):
                fault = 'No Fault '
                if extract(data,motor['AmplifierFault']):
                    fault = 'Amp Fault'
                limit = 'None '
                if extract(data,motor['MinusLimit']):
                    limit = 'LoLim'
                if extract(data,motor['PlusLimit']):
                    limit = 'HiLim'
                line = motstr[j]+fault+'  '+limit+'  {:5.1f}    {:5.1f}   {:5.1f}   {:5.1f}'.format(
                  extract(data,motor['MotorCurrent']),extract(data,motor['Position']),
                  extract(data,motor['PositionError']),extract(data,motor['PositionOffset']))
                self.cryoLB.insert(END,line)
            line = ''
            self.cryoLB.insert(END,line)
            self.cryoLB.insert(END,line)
        ...
        for k,i in enumerate(antindex):
            ...
            criotdif = (criomjd - mjd)*86400.
            anttdif = (antmjd - mjd)*86400.
            line = antn[k]+pswitch[np.clip(extract(data,c['PowerSwitch']),0,2)]
            line += "  "+rctrl[np.clip(extract(data,c['RunControl']),0,2)]
            line += "  "+rmode[np.clip(extract(data,c['RunMode']),0,5)]
            line += "  "+dmode[np.clip(extract(data,c['DataMode']),0,2)]
            line += "  "+criotimestr[11:]+"  "+anttimestr[11:]+'{:8.3f}  {:8.3f}'.format(criotdif,anttdif)
...
#        daydif = (datetime.datetime(1901,1,1)-datetime.datetime(1,1,1)).days
        ...
        for i in range(npts):
            amb[i] = extract(self.que[i],sf['Schedule']['Data']['Weather']['Temperature'])
            tm[i] = extract(self.que[i],sf['Timestamp'])
            for j in range(13):
                temps[i,j,0] = extract(self.que[i],laird[j]['Temperature'])
                temps[i,j,1] = extract(self.que[i],fe[j]['Temperature'])
            temps[i,13,0] = extract(self.que[i],laird[13])
            temps[i,13,1] = extract(self.que[i],fe[13]['Temperature'])
            wind[i] = extract(self.que[i],windkey)
...
#            self.canvas.draw()
            ...
            for locator in self.miscplotlocator:
                ...
                for i in range(npts):
                    item = extract(self.que[i],locator)
                    if type(item) is list:
                        item = item[0]
                    plots[-1][i] = item
...
#=========================
    def handle_Extra_plots(self):
        ...
        for locator in temp_plots:
            plots.append(np.zeros(npts,'float'))
            for i in range(npts):
                plots[-1][i] = extract(self.que[i],locator)
            plots[-1][np.where(plots[-1] == 0.0)[0]] = np.NaN
