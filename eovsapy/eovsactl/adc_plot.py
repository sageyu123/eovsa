...
class AGC_Thread (threading.Thread):
    ...
    def get_FEM_level(self):
        ...
        if msg == 'No Error':
            hpol1 = extract(buf,accini['sf']['Antenna'][a1]['Frontend']['FEM']['HPol']['Regime']['Level'])
            vpol1 = extract(buf,accini['sf']['Antenna'][a1]['Frontend']['FEM']['VPol']['Regime']['Level'])
            hpol2 = extract(buf,accini['sf']['Antenna'][a2]['Frontend']['FEM']['HPol']['Regime']['Level'])
            vpol2 = extract(buf,accini['sf']['Antenna'][a2]['Frontend']['FEM']['VPol']['Regime']['Level'])
            agc1 = extract(buf,accini['sf']['Antenna'][a1]['Frontend']['FEM']['AGC']['Active'])
            agc2 = extract(buf,accini['sf']['Antenna'][a2]['Frontend']['FEM']['AGC']['Active'])
...
def adc_monitor(nloop=None, verbose=False):
    ...
    #        stdev[-1,i]=t.sd
        ...
        buf, msg = stf.get_stateframe(accini)
        if msg == 'No Error':
            fseqfile = extract(buf,accini['sf']['LODM']['LO1A']['FSeqFile']).strip('\0')
        else:
            fseqfile = 'Unknown'
...
#    fh.close()
    if attnval is None:
        ans = input('Enter the fixed DCM attn value used (integer dB, or q to quit): ')
        if ans.upper() == 'Q':
            return
        try:
            attnval = int(ans)
    ...
    # Replaced by:
    ans = input('Do you want to save this to a DCM master table file? [y/n]?')
    if ans.upper() == 'Y':
        timestr = time.iso
