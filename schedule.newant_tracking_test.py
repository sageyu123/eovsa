...
#                    starburst.execute_ctlline(self,ctlline,sh_dict,sf_dict,mjd1,mjd2)
                ...
                if ctlline.split()[0].upper() == '$MK_TABLES':
                    ...
                    if fname.upper() == 'GEOSAT_TAB':
                        tbls[fname_base] = make_geosattable(sf_dict['geosat'],self.aa,mjd1,mjd2)
                    else:
                        tbls[fname_base] = make_tracktable(src,self.aa,mjd1,mjd2)
                        # For the new-antenna tracking test, generate shifted tables for
                        # ants 9, 10, 11, and 13 while leaving the default table unchanged
                        ant_shifts = {9: 15., 10: 30., 11: 45., 13: 60.}
                        for antidx, shift_sec in ant_shifts.items():
                            shift_day = shift_sec / 86400.
                            tbls[fname_base+'_ant{:02d}'.format(antidx)] = make_tracktable(
                                src, self.aa, mjd1 + shift_day, mjd2 + shift_day
                            )
                    for tbl_name, tbl in tbls.items():
                        # Write out to file with .radec extension
                        fname = tbl_name+'.radec'
                        f = open('/tmp/'+fname,'w')
                        f.write(tbl)
                        f.close()
                        time.sleep(0.01)
