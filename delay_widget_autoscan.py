...
class App():
    ...
    def scan_xdelay(self):
        ...
        Notes:
          * This routine does not modify any other existing UI behavior.
          * It scans only the current antenna's X-delay.
          * It uses the same XX residual expression as the XX panel uses.
        '''
        ant_str = self.ant.get()
        try:
            ant = int(ant_str)
