...
def xdata_display(t, ax=None):
    ...
    if len(fidx) > 0:
        files = fdb['FILE'][fidx]
        files_st = Time(fdb['ST_TS'][fidx].astype(np.float), format='lv')
        ...
        if len(projdict['EOS']) == nscans:
            for i in range(nscans):
                ...
                if projdict['Project'][i] == 'NormalObserving':
                    plt.plot_date(uti, yran[1] * np.array([0.93, 0.93]), ls='-', marker='None', color='#aaffaa', lw=2,
                                  solid_capstyle='butt')
                elif projdict['Project'][i][:4] in known:
                    plt.plot_date(uti, yran[1] * np.array([0.95, 0.95]), ls='-', marker='None', color='#aaaaff', lw=2,
                                  solid_capstyle='butt')
                else:
                    plt.plot_date(uti, yran[1] * np.array([0.97, 0.97]), ls='-', marker='None', color='#ffaaaa', lw=2,
                                  solid_capstyle='butt')
